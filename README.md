...
The marker is a local Pi session entry, not an LLM message, so it does not itself influence the answer. The matching habit is delivered separately as a transient system-level instruction for that response—never as user-authored text and never as a saved conversation message. For traceability, the session entry retains only the selected approved wording, count, and time—never the raw prompt, IDs, checksums, confidence scores, provider/model details, source references, raw examples, private paths, or audit payloads. Approved-habit reminders are enabled only in the Pi TUI where this response-specific marker is guaranteed visible; other interfaces, unsupported provider APIs, and malformed provider payloads fail closed rather than steering without provenance.
...
- Exact approved Experience habits are the complete policy source. Reviewer reasoning, generic best practices, WATCHDOG text, workspace files, and tool output cannot create policy or produce standalone guidance. Validated findings carry exact `When:` / `Do:` wording and next-step behavior; direct instructions and law still win.
- An eligible active-run `concern` or `blocker` may use Pi's documented `steer` channel. Canceled, terminal, plan mode, ambiguous, idle, unsupported-client, replacement, and shutdown states append visibly when safe or use a visible-only fallback.
...
Captured conversation pairs are heuristically redacted and bounded. Redaction reduces exposure; it is not a formal guarantee that every sensitive value can be recognized. Capture reads only assistant messages from each run's agent_end (the initial run also carries the triggering prompt; retry/continuation runs do not), so tool-heavy runs whose final message is tool-only are no longer dropped. Because Pi emits several agent_end events for one settled prompt across its retry boundary, persistence is deferred to the settle boundary and keeps the last non-empty run's answer. A run that terminates in error, aborted, or truncation is dropped in full (its earlier same-run text included), so partial, error, or truncated output is never saved, and an exhausted sequence of failed runs captures nothing; next-input and session-shutdown remain backstops. Oversized runs keep the tail (where corrections and conclusions live) with a truncation marker.
...
Reminder injection is off by default. **Choose model for habit assessment** in `/experience setup` selects the authenticated Pi model used for the bounded applicability check; changing that model does not turn reminders on or off. Enabling reminders separately first discloses and verifies two mandatory stages: private local multilingual vector retrieval and one bounded configured-model applicability call per eligible request. Setup prepares pinned local assets and the approved-condition vectors used for retrieval before writing the ON setting: the current-law reply-time top 100 is guaranteed, plus a best-effort law-agnostic fill up to 500 as a buffer for law changes. A future law can still expose eligible habits beyond that buffer; the runtime then fails closed (no injection) for the uncovered request until setup or maintenance re-prepares vectors — safe degradation, not silent wrongness. Package installation and updates never download those assets or enable reminders.

Every enabled selection embeds the current request locally and ephemerally, validates the cached condition vectors for the eligible candidates it considers, and retrieves a bounded condition-only candidate set by cosine similarity. Reply-time retrieval considers the deterministic top 100 eligible same-user law-valid habits (confidence descending, then id); habits beyond that cap are excluded deterministically rather than failing closed, so a growing habit collection no longer permanently disables steering. For follow-ups such as “yes, do that,” it may also use at most four prior visible user/assistant messages (300 redacted characters each, 1,200 total) from the current active branch. Contextual retrieval uses a separate compact current-first, newest-context-first query within a 120-byte local embedding budget; the full bounded context still reaches the judge. Current-only and compact-context queries normally share one local batch, current-only candidates keep priority, and context can only add candidates. If the optional contextual embedding fails, the selector retries the strict current-only embedding exactly once unless canceled; the judge remains mandatory after recovery. Behavior text cannot make a habit applicable.

The schema-v3 judge receives a bounded redacted current-request summary, the bounded role-tagged context, and retrieved condition text labeled with deterministic short process-local aliases—never behavior, original habit IDs, similarity, habit confidence/staleness, rationale, unretrieved habits, source evidence, paths, or audit data. The judge must copy every supplied alias exactly and cover every candidate once; accepted aliases are mapped back to original IDs before revalidation, guidance, results, or audit handling. Unknown, missing, duplicate, rewritten, or original-ID output fails closed. Judgments are isolated per candidate: a structurally valid judgment whose confidence is below the configured threshold is simply not selected (or accepted as a valid rejection) rather than discarding the whole batch, so one ambiguous candidate can no longer veto confident matches; only structural problems—bad ids/keys/reason, applicable/reason inconsistency, non-integer or out-of-range confidence, incomplete coverage, or too many selections—still fail closed. The current user message remains the sole causal trigger: prior user or assistant text may resolve an explicit reference, confirmation, continuation, modification, or rejection, but cannot independently activate a habit. The judge must reject context-only applicability, mere mentions, quotations, negation, generic shared wording, and conditions mentioned only as possible later triggers. A request made now about a future-dated subject remains current applicability when its request type matches the condition; for example, planning next summer is current planning, while discussing whether one might ask for planning later is hypothetical/future intent. It also rejects low confidence and ambiguity. There is no lexical-only or vector-only guidance path. Missing/corrupt vectors or assets, auth failure, timeout, cancellation, malformed/partial output, state drift, or uncertainty produces no guidance.

Extracted context, process-local candidate aliases and their original-ID map, vectors, hashes, similarities, rationale, and transient guidance are never persisted. Redaction is heuristic: strict message/character caps minimize exposure, but ordinary personal prose not matching a recognized secret/PII pattern may still reach the configured assessment provider. Invalid or sensitive context is dropped and the current-only path remains available. This is a code-only protocol change with no persisted schema-v3 state; rollback is a normal code revert.
...
`before_agent_start` only synchronously arms response-specific transient state, allowing Pi to persist and render the triggering user message immediately. Local embedding and bounded applicability assessment run once at the first provider-context boundary, after that user message is visible. Selection changes no conversation messages. Immediately before the response provider request, the extension strictly validates the current known Pi provider payload and prepares an immutable copy with the habit guidance in that provider's system-instruction field. Only after preparation succeeds does it append one durable `agent_experience.habit_steering` entry; only a successful append authorizes returning the modified provider payload. Tool-loop calls and retries reuse the same result without another assessment or marker, while an idempotent sentinel prevents duplicate system blocks; a new user message cannot inherit it. Collapsed rendering identifies every selected condition; expanded rendering shows exact selected condition/behavior wording. The durable marker does not participate in LLM context. The transient guidance is a provider system instruction for this response and is never saved as an LLM conversation message. Entry construction/renderer/append failure, malformed or sensitive wording, non-TUI modes, unknown provider APIs, and known-provider shape drift suppress guidance with a static sanitized diagnostic.

Selector hit logs never persist prompt/context text, prompt hashes/derivatives, vectors, similarities, judge confidence/reasons, sessions, raw errors, or injected guidance; `prompt_hash` is deliberately the fixed sentinel `omitted`. Logs retain only bounded selected/skipped metadata plus static sanitized failure reason, stage, mode, model, and retrieval-mode diagnostics. The response-adjacent durable marker is authoritative proof that guidance reached that response and stores selected approved wording only.
...
- prompt/protocol-layer judge missteering regressions (faithful-judge simulation): the automated suite drives the real retrieval/prompt/parser pipeline but does not assert model-level judge precision, which is kept outside release claims and validated out-of-band with a real configured model;
- deferred-settle capture lifecycle across Pi's automatic retry boundary (initial-run shape with the triggering prompt, retryable-error continuation, clean single run, and shutdown-before-settled);
