...
```bash
npm pack --dry-run
npm pack --json --pack-destination /tmp/pi-experiences-043-pack
```

The tarball must include:

- `package.json` version `0.1.43` and Node floor `>=22.19.0`;
...
```bash
npm install --ignore-scripts --legacy-peer-deps /tmp/pi-experiences-043-pack/pi-experiences-0.1.43.tgz
```
...
```bash
AX_STATE_ROOT=/tmp/pi-experiences-043-tui-smoke-state
```
