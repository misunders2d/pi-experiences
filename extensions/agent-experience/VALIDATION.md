...
```bash
npm pack --dry-run
npm pack --json --pack-destination /tmp/pi-experiences-042-pack
```

The tarball must include:

- `package.json` version `0.1.42` and Node floor `>=22.19.0`;
...
```bash
npm install --ignore-scripts --legacy-peer-deps /tmp/pi-experiences-042-pack/pi-experiences-0.1.42.tgz
```
...
```bash
AX_STATE_ROOT=/tmp/pi-experiences-042-tui-smoke-state
```
