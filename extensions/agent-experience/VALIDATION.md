...
```bash
npm pack --dry-run
npm pack --json --pack-destination /tmp/pi-experiences-040-pack
```
...
```bash
npm install --ignore-scripts --legacy-peer-deps /tmp/pi-experiences-040-pack/pi-experiences-0.1.40.tgz
```
...
```bash
AX_STATE_ROOT=/tmp/pi-experiences-040-tui-smoke-state
```
