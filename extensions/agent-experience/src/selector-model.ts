...
const SELECTOR_SYSTEM_PROMPT = [
	...
	"Use hypothetical_or_future only when the current message merely discusses a possible later trigger instead of triggering the condition now. Use not_currently_relevant only when the present request does not semantically instantiate the condition.",
	"Example: condition 'When I mention or ask about a trip' plus request 'Plan my vacation for next summer' is current_applicability. The same condition plus 'If I ask you to plan a trip next month, what would happen?' is hypothetical_or_future.",
