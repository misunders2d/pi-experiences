...
let setupChoices = [];
let setupInputs = [];
const availableModel = { provider: 'openai-codex', id: 'gpt-5.5', name: 'GPT 5.5', api: 'openai-completions', input: ['text'] };
...
assert.equal(activeContext, undefined, 'response context must not receive user/custom habit guidance');
const activeProvider = handlers.get('before_provider_request')({ payload: { messages: [{ role: 'developer', content: 'base system' }, { role: 'user', content: activePrompt }] } }, ctx);
const activeGuidance = activeProvider.messages[0].content;
assert.match(activeGuidance, /approved habit guidance|Answer concisely/i, 'provider system instruction should receive approved habit guidance only after marker commit');
assert.doesNotMatch(activeGuidance, /please be concise|too much fluff|again: concise/, 'selector injection must not include raw saved examples');
assert.doesNotMatch(JSON.stringify(activeProvider.messages[1]), /agent_experience_response_guidance|Agent Experience approved habit guidance/i, 'habit guidance framing must not enter provider user content');
