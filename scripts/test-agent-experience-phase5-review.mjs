...
try {
  ...
  assert.match(notes.at(-1).message, /habits report/);
  assert.match(notes.at(-1).message, /Selector defaults disabled/);
  assert.match(notes.at(-1).message, /embeddings, timers/);
  ...
  assert.match(notes.at(-1).message, /experience capture on/);
  await commands.get('experience').handler('help review', { cwd: process.cwd(), ui: { notify(message, level) { notes.push({ message, level }); } } });
  assert.match(notes.at(-1).message, /Agent Experience review commands/);
  assert.match(notes.at(-1).message, /pending accept/);
