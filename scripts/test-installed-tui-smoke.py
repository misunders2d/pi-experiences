...
state=Path(os.environ.get('AX_STATE_ROOT','/tmp/pi-experiences-033-tui-smoke-state')).resolve(); state.mkdir(parents=True,exist_ok=True)
work=state.parent/'pi-experiences-0.1.33-tui-work'; work.mkdir(parents=True,exist_ok=True)
...
try:
    ...
    initial=text(mark)
    for label in ['Save chat examples locally','Choose model for habit learning','Analyze saved examples now','Review suggested habits','Resolve duplicate habits','Review approved habits','Prevent duplicate habits','Keep analyzed source examples','Use approved habits before replies','Automatic schedule','Show current settings','Explain these settings','Done']:
    ...
    mark=len(raw); enter(fd); wait(fd,r'Save chat examples locally: ON',start=mark); drain(fd,.5); down(fd,12); mark=len(raw); enter(fd); wait(fd,r'Agent Experience is OFF',start=mark); drain(fd,.5)
