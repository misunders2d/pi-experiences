...
state=Path(os.environ.get('AX_STATE_ROOT','/tmp/pi-experiences-046-tui-smoke-state')).resolve(); shutil.rmtree(state,ignore_errors=True); state.mkdir(parents=True,exist_ok=True)
work=state.parent/'pi-experiences-0.1.46-tui-work'; work.mkdir(parents=True,exist_ok=True)
...
try:
    ...
    down(fd,11); mark=len(raw); enter(fd); wait(fd,r'Explain break-in review prompts',start=mark); mark=len(raw); enter(fd); wait(fd,r'Break-in review prompts are private TUI reminders',start=mark); drain(fd,.5)
    ...
    mark=len(raw); enter(fd); wait(fd,r'Save chat examples locally\s+\[x\] ON',start=mark); drain(fd,.5); down(fd,14); mark=len(raw); enter(fd); wait(fd,r'Agent Experience is OF',start=mark); drain(fd,.5)
